...
signal = darrow(P(0,0), P(.3,1.7), TeX('\small signal'), 
                                   fg=Color('green'), bg=Color('green'))

# double-headed arrow for the activation
activation = darrow(P(.6,1.7), P(1.8,.3), TeX('\small activation'), 
                                   fg=Color('red'), bg=Color('red'))

# double-headed arrow for the read out
read_out = darrow(P(2.2,.3), P(2.5,2.5), TeX('\small read out'), 
                                   fg=Color('blue'), bg=Color('blue'))

diagram = Group(levels, detuning, signal, activation, read_out)
